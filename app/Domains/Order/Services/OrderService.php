    ...
    public function create(array $data, ?Cart $cart = null, ?User $user = null): Order
    {
        ...
        try {
            return DB::transaction(function () use ($data, $cart, $user) {
                ...
                // We iterate over the line items generated by the pricing engine, as they
                // contain the authoritative list of what is being purchased/gifted.
                foreach ($pricing->lineItems as $lineItem) {
                    if (!empty($lineItem['combo_id'])) {
                        // For combos, we must reserve stock for every component variant.
                        $combo = \App\Domains\Product\Models\Combo::with('items')->findOrFail($lineItem['combo_id']);
                        ...
                        // For regular items and auto-injected gifts, we reserve for the specific variant.
                        $pricing->lockedVariants
