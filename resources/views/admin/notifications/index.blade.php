    ...
    <div x-data="notificationCenter()" x-init="init()" x-cloak>
        ...
        <div class="border-b border-champagne mb-6">
            <nav class="-mb-px flex gap-6">
                ...
                <button @click="switchTab('notifications')"
                    :class="tab === 'notifications' ? 'border-indigo-600 text-indigo-600' :
                        'border-transparent text-muted hover:text-brown'"
                    class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
                ...
                <button @click="switchTab('failed')"
                    :class="tab === 'failed' ? 'border-indigo-600 text-indigo-600' :
                        'border-transparent text-muted hover:text-brown'"
                    class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
                ...
                @can('notification.send')
                    <button @click="switchTab('send')"
                        :class="tab === 'send' ? 'border-indigo-600 text-indigo-600' :
                            'border-transparent text-muted hover:text-brown'"
                        class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
            ...
            <div x-show="!statsLoading && stats" class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                    ...
                    <p class="text-2xl font-bold text-brand"
                        x-text="stats ? stats.total_notifications.toLocaleString() : 0"></p>
                    <p class="text-xs text-taupe mt-1">All time</p>
                </div>
                {{-- Today Sent --}}
                <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                    <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Today</p>
                    <p class="text-2xl font-bold text-indigo-600" x-text="stats ? stats.today_sent.toLocaleString() : 0">
                    </p>
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                    ...
                    <p class="text-2xl font-bold text-amber-600"
                        x-text="stats ? stats.unread_notifications.toLocaleString() : 0"></p>
        ...
        <div x-show="tab === 'notifications'">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-champagne p-4 mb-4 flex flex-wrap gap-3">
                <div class="relative flex-1 min-w-[200px]">
                    ...
                    <input x-model="notifSearch" @input.debounce.400ms="notifPage = 1; loadNotifications()" type="text"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-champagne overflow-hidden">
                ...
                <table x-show="!notifLoading" class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-cream">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">
                                Recipient</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">
                                Subject</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">Type
                            </th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">Status
                            </th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">Sent
                            </th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-cream transition">
                                ...
                                <td class="px-5 py-3.5">
                                    <span :class="n.read_at ? 'bg-cream text-gold-antique' : 'bg-amber-100 text-amber-700'"
                                        ...
                                        <span :class="n.read_at ? 'bg-ivory' : 'bg-amber-500'"
        ...
        <div x-show="tab === 'failed'">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-champagne p-4 mb-4 flex flex-wrap gap-3 items-center">
                <div class="relative flex-1 min-w-[200px]">
                    <i class="fas fa-search absolute left-3 top-2.5 text-taupe text-sm"></i>
                    <input x-model="failedSearch" @input.debounce.400ms="failedPage = 1; loadFailedJobs()" type="text"
                        placeholder="Search job name, queue, exception…"
                        class="pl-9 pr-4 py-2 w-full text-sm border border-champagne rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                </div>
                ...
                @can('notification.manage')
                    <div class="ml-auto flex gap-2">
                        <button @click="confirmRetryAll()" :disabled="failedMeta.total === 0"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-champagne overflow-hidden">
                ...
                <table x-show="!failedLoading" class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-cream">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">Job
                            </th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">Queue
                            </th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">
                                Exception</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase tracking-wide">Failed
                                At</th>
                            <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase tracking-wide">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        <template x-for="job in failedJobs" :key="job.id">
                            <tr class="hover:bg-cream transition">
                                <td class="px-5 py-3.5">
                                    ...
                                    <p class="text-xs text-taupe font-mono mt-0.5"
                                        x-text="job.uuid.substring(0,13) + '…'"></p>
                                </td>
                                <td class="px-5 py-3.5">
                                ...
                                <td class="px-5 py-3.5 max-w-xs">
                                    <p class="text-xs text-red-600 font-mono truncate"
                                        x-text="job.exception_short.split('\n')[0]"></p>
                                ...
                                <td class="px-5 py-3.5 text-right">
                                    @can('notification.manage')
                                        <div class="flex items-center justify-end gap-2">
                                            <button @click="retryJob(job.uuid)" :disabled="retryingJob === job.uuid"
                        ...
                        <form @submit.prevent="sendNotification" class="space-y-5">
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2 mb-3">
                                    <template
                                        x-for="opt in [{v:'all',l:'All Active Users'},{v:'role',l:'By Role'},{v:'specific',l:'Specific IDs'}]"
                                        :key="opt.v">
                                        <button type="button"
                            ...
                            <div>
                                ...
                                <input x-model="sendForm.subject" type="text" maxlength="200"
                                    ...
                                    class="w-full text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                                <p class="text-xs text-taupe mt-1 text-right"
                                    x-text="(sendForm.subject?.length || 0) + '/200'"></p>
                            </div>

                            {{-- Message --}}
                            <div>
                                <label class="block text-sm font-medium text-brown mb-1">Message</label>
                                <textarea x-model="sendForm.message" rows="5" maxlength="5000" placeholder="Write your message here…"
                                ...
                                <p class="text-xs text-taupe mt-1 text-right"
                                    x-text="(sendForm.message?.length || 0) + '/5000'"></p>
                            </div>
                            ...
                            <div class="flex items-center justify-between pt-1">
                                <button type="button" @click="resetSendForm()" class="text-sm text-muted hover:text-brown">
                                    <i class="fas fa-rotate-left mr-1"></i> Clear form
                                </button>
                                <button type="submit"
                                    :disabled="sendLoading || sendForm.channels.length === 0 || !sendForm.subject || !sendForm
                                        .message"
        ...
        <div x-show="showDeleteJobModal" x-transition.opacity
            ...
            <div @click.outside="showDeleteJobModal = false; deleteJobData = null"
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button @click="deleteJob(deleteJobData.uuid)" :disabled="deletingJob === deleteJobData?.uuid"
        ...
        <div x-show="showRetryAllModal" x-transition.opacity
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
            <div @click.outside="showRetryAllModal = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                <div class="flex items-start gap-3 mb-4">
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button @click="retryAllJobs()" :disabled="retryingAll"
                ...
                async sendNotification() {
                    ...
                    try {
                        ...
                        if (r.ok && data.success) {
                            ...
                            this.sendError = data.message || (data.errors ? Object.values(data.errors).flat().join(
                                ', ') : 'Failed to send');
                        }
                    } catch (e) {
