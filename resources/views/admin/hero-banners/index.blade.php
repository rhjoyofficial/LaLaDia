...
<div x-data="heroBannersApp()" x-init="init()" x-cloak>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-brand">Hero Banners</h1>
            <p class="text-sm text-muted mt-0.5">Manage homepage hero section banners</p>
        </div>
    ...
    <div x-show="flashMsg" x-cloak class="mb-4 rounded-xl border px-5 py-3 text-sm"
         :class="flashType === 'success' ? 'bg-ivory border-sand text-gold-antique' : 'bg-red-50 border-red-200 text-red-700'">
    ...
    <div x-show="loading" class="space-y-3">
        <template x-for="i in 3">
            <div class="bg-white rounded-xl border border-champagne p-4 animate-pulse flex gap-4">
    ...
    {{-- Banners Table --}}
    <div x-show="!loading" class="bg-white border border-champagne rounded-xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-cream border-b border-champagne">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted uppercase">Image</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted uppercase">Banner</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted uppercase">Status</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted uppercase">Order</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted uppercase">Schedule</th>
                        ...
                        <tr class="hover:bg-cream transition">
                            ...
                            <td class="px-4 py-3">
                                <template x-if="banner.badge">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-cream text-gold-antique mb-1 mr-1" x-text="banner.badge"></span>
                                </template>
                                <p class="font-semibold text-brown" x-text="banner.title"></p>
                                <p class="text-xs text-taupe mt-0.5 line-clamp-1" x-text="banner.subtitle || banner.description || '—'"></p>
                            </td>
                            <td class="px-4 py-3">
                                <button @click="toggleActive(banner)"
                                    :class="banner.is_active ? 'bg-cream text-gold-antique' : 'bg-gray-100 text-muted'"
                            ...
                            <td class="px-4 py-3 text-sm text-muted font-mono" x-text="'#' + banner.sort_order"></td>
                            <td class="px-4 py-3 text-xs text-taupe">
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4 py-8 overflow-y-auto">
        <div @click.away="closeModal()" class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl my-auto">
            ...
            <div class="px-6 py-4 border-b border-champagne flex items-center justify-between">
                <h3 class="font-bold text-brown" x-text="editingId ? 'Edit Hero Banner' : 'Create Hero Banner'"></h3>
                <button @click="closeModal()" class="text-taupe hover:text-muted cursor-pointer text-xl leading-none">&times;</button>
            </div>
            ...
            <form @submit.prevent="saveBanner()" enctype="multipart/form-data">
                <div class="p-6 space-y-5">
                    ...
                    {{-- Image Upload --}}
                    <div>
                        <label class="block text-xs font-medium text-brown mb-2">Banner Image</label>
                        <div class="flex items-start gap-4">
                            <div class="w-32 h-20 rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center shrink-0 border border-champagne">
                            ...
                            <div class="flex-1">
                                <input type="file" x-ref="imageInput" accept="image/*"
                                    class="block w-full text-sm text-muted file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-ivory file:text-gold-antique hover:file:bg-cream cursor-pointer"
                                    @change="handleImagePreview">
                                <p class="text-xs text-taupe mt-1">JPG, PNG, WebP — max 4MB</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-xs font-medium text-brown mb-1">Title <span class="text-red-500">*</span></label>
                        ...
                        {{-- Badge --}}
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">Badge</label>
                        ...
                        {{-- Subtitle --}}
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">Subtitle</label>
                        ...
                        {{-- Description --}}
                        <div class="md:col-span-2">
                            <label class="block text-xs font-medium text-brown mb-1">Description</label>
                            <textarea x-model="form.description" rows="2"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary resize-none"
                                placeholder="Short supporting copy displayed beneath the title..."></textarea>
                        </div>

                        {{-- Button Text --}}
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">Button Text</label>
                        ...
                        {{-- Button URL --}}
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">Button URL</label>
                        ...
                        {{-- Sort Order --}}
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">Sort Order</label>
                        ...
                        <div class="flex items-center gap-3 pt-5">
                            <span class="text-xs font-medium text-brown">Active</span>
                        ...
                        {{-- Schedule --}}
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">Start Date (optional)</label>
                            <input x-model="form.starts_at" type="datetime-local"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-brown mb-1">End Date (optional)</label>
                ...
                {{-- Modal Footer --}}
                <div class="px-6 py-4 bg-cream border-t border-champagne flex items-center justify-end gap-3">
                    <button type="button" @click="closeModal()"
                        class="px-4 py-2 text-sm font-medium text-brown bg-white border border-gray-300 rounded-lg hover:bg-cream transition cursor-pointer">
                        Cancel
                    </button>
