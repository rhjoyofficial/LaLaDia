...
@section('content')
    <div x-data="transactionCenter()" x-init="init()" x-cloak>
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-brand">Transactions & Reconciliation</h1>
                <p class="text-sm text-muted mt-0.5">Revenue overview, payment ledger, and discrepancy detection</p>
            </div>
        ...
        <div class="border-b border-champagne mb-6">
            <nav class="-mb-px flex gap-6">
                <button @click="switchTab('summary')"
                    :class="tab === 'summary' ? 'border-indigo-600 text-indigo-600' :
                ...
                <button @click="switchTab('ledger')"
                    :class="tab === 'ledger' ? 'border-indigo-600 text-indigo-600' :
                        'border-transparent text-muted hover:text-brown'"
                    class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
                ...
                <button @click="switchTab('reconciliation')"
                    :class="tab === 'reconciliation' ? 'border-indigo-600 text-indigo-600' :
                        'border-transparent text-muted hover:text-brown'"
                    class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
            ...
            <div x-show="!summaryLoading && summary">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Total Revenue</p>
                        <p class="text-2xl font-bold text-brand" x-text="fmt(summary.totals?.revenue)"></p>
                        <p class="text-xs text-taupe mt-1">All paid orders</p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Net Revenue</p>
                        <p class="text-2xl font-bold text-primary" x-text="fmt(summary.totals?.net_revenue)"></p>
                        <p class="text-xs text-taupe mt-1">After refunds</p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        ...
                        <p class="text-xs text-taupe mt-1">Month-to-date</p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        ...
                        <p class="text-xs text-taupe mt-1">Today's revenue</p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Total Refunds</p>
                        <p class="text-2xl font-bold text-red-600" x-text="fmt(summary.totals?.total_refunds)"></p>
                        <p class="text-xs text-taupe mt-1">Refunded amount</p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Unpaid Orders</p>
                        <p class="text-2xl font-bold text-amber-600" x-text="summary.totals?.unpaid_count"></p>
                        <p class="text-xs text-taupe mt-1" x-text="fmt(summary.totals?.unpaid_total) + ' outstanding'"></p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Failed Payments</p>
                        <p class="text-2xl font-bold"
                            :class="summary.totals?.failed_count > 0 ? 'text-red-600' : 'text-brand'"
                            x-text="summary.totals?.failed_count"></p>
                        <p class="text-xs text-taupe mt-1">Need resolution</p>
                    </div>
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne cursor-pointer hover:border-red-300 transition"
                        @click="switchTab('reconciliation')">
                        <p class="text-xs font-medium text-muted uppercase tracking-wide mb-1">Discrepancies</p>
                        <p class="text-2xl font-bold" :class="summary.discrepancies > 0 ? 'text-red-600' : 'text-primary'"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="lg:col-span-2 bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <h3 class="text-sm font-semibold text-brown mb-4">Daily Revenue — Last 30 Days</h3>
                        <canvas id="revenueChart" height="90"></canvas>
                    </div>

                    {{-- Payment Method Breakdown --}}
                    <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                        <h3 class="text-sm font-semibold text-brown mb-4">By Payment Method</h3>
                        <div x-show="(summary.by_method ?? []).length === 0" class="text-sm text-taupe text-center py-8">No
                            data</div>
                        <template x-for="m in (summary.by_method ?? [])" :key="m.method">
                            <div class="mb-4">
                                <div class="flex justify-between text-sm mb-1">
                                    <span class="font-medium text-brown capitalize"
                                        x-text="m.method === 'cod' ? 'Cash on Delivery' : 'SSLCommerz'"></span>
                                    <span class="text-muted" x-text="m.count + ' orders'"></span>
                                </div>
                                <div class="w-full bg-gray-100 rounded-full h-2">
                                    <div class="h-2 rounded-full bg-indigo-500" :style="'width:' + methodPct(m) + '%'">
                                    </div>
                                </div>
                                <p class="text-xs text-muted mt-1" x-text="fmt(m.total)"></p>
                            </div>
                        </template>

                        {{-- Transaction type breakdown --}}
                        <div class="border-t border-champagne pt-4 mt-2">
                            <h4 class="text-xs font-medium text-muted uppercase tracking-wide mb-3">By Type</h4>
                            <template x-for="[type, data] in Object.entries(summary.by_type ?? {})" :key="type">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <div class="text-right">
                                        <span class="text-xs font-medium text-brown" x-text="fmt(data.total)"></span>
                                        <span class="text-xs text-taupe ml-1" x-text="'(' + data.count + ')'"></span>
                                    </div>
                                </div>
                            </template>
                        </div>
        ...
        <div x-show="tab === 'ledger'">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-champagne p-4 mb-4 flex flex-wrap gap-3 items-center">
                <div class="relative flex-1 min-w-[220px]">
                    ...
                    <input x-model="ledgerSearch" @input.debounce.400ms="ledgerPage=1; loadLedger()" type="text"
                ...
                <input x-model="ledgerFrom" @change="ledgerPage=1; loadLedger()" type="date"
                    class="text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500"
                    title="From date">
                <input x-model="ledgerTo" @change="ledgerPage=1; loadLedger()" type="date"
                    class="text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500"
                    title="To date">
                ...
                <table x-show="!ledgerLoading" class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-cream">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">Description</th>
                            <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">Amount</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        <template x-for="tx in ledger" :key="tx.id">
                            <tr class="hover:bg-cream transition cursor-pointer" @click="openOrderPanel(tx.order_id)">
                                <td class="px-5 py-3.5 text-sm text-muted whitespace-nowrap"
                                    x-text="fmtDate(tx.created_at)"></td>
                                <td class="px-5 py-3.5">
                                    <p class="text-sm font-medium text-indigo-600" x-text="tx.order_number"></p>
                                    <p class="text-xs text-taupe mt-0.5" x-text="tx.order?.customer_name ?? ''"></p>
                                </td>
                                <td class="px-5 py-3.5">
                                ...
                                <td class="px-5 py-3.5 text-sm text-muted max-w-xs truncate"
                                    x-text="tx.description ?? '—'"></td>
                                <td class="px-5 py-3.5 text-right">
                                    <span
                                        :class="tx.type === 'refund' || tx.type === 'discount' || tx.type === 'coupon' ?
                                            'text-red-600' : 'text-gold-antique'"
                                        class="text-sm font-semibold">
                                        <span
                                            x-text="tx.type === 'refund' || tx.type === 'discount' || tx.type === 'coupon' ? '−' : '+'"></span>
        ...
        <div x-show="tab === 'reconciliation'">
            ...
            <div x-show="reconCounts" class="flex flex-wrap gap-2 mb-4">
                <button @click="reconIssue=''; reconPage=1; loadReconciliation()"
                    :class="reconIssue === '' ? 'bg-gray-800 text-white' :
                        'bg-white text-muted border border-champagne'"
                    class="px-3 py-1.5 text-xs font-medium rounded-lg transition">
                    All Issues
                </button>
                <template x-for="[key, label] in issueTypes" :key="key">
                    <button @click="reconIssue = key; reconPage=1; loadReconciliation()"
                        :class="reconIssue === key ? issueActiveClass(key) : 'bg-white text-muted border border-champagne'"
                        class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-lg transition">
                        <span x-text="label"></span>
                        <span x-show="reconCounts[key] > 0" class="bg-white/30 px-1 rounded-sm text-xs font-bold"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-champagne overflow-hidden">
                ...
                <table x-show="!reconLoading" class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-cream">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">Order</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">Method</th>
                            <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">Grand Total</th>
                            <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">Charged</th>
                            <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">Net Received</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">Issue</th>
                        ...
                        <template x-for="row in reconRows" :key="row.id">
                            <tr class="hover:bg-cream transition">
                                <td class="px-5 py-3.5">
                                    ...
                                    <p class="text-xs text-muted mt-0.5" x-text="row.customer_name"></p>
                                    <p class="text-xs text-taupe" x-text="fmtDate(row.placed_at)"></p>
                                </td>
                                <td class="px-5 py-3.5">
                                    <span
                                        class="text-xs bg-gray-100 text-muted px-2 py-0.5 rounded-full font-medium uppercase"
                                        x-text="row.payment_method"></span>
                                    <p x-show="row.gateway_transaction_id"
                                        class="text-xs text-taupe font-mono mt-1 truncate max-w-[120px]"
                                        x-text="row.gateway_transaction_id"></p>
                                </td>
                                <td class="px-5 py-3.5 text-right text-sm font-semibold text-brown"
                                    x-text="fmt(row.grand_total)"></td>
                                <td class="px-5 py-3.5 text-right text-sm"
                                    :class="parseFloat(row.charged) >= parseFloat(row.grand_total) ? 'text-gold-antique' :
                                        'text-red-600'"
                                    x-text="fmt(row.charged)"></td>
                                <td class="px-5 py-3.5 text-right text-sm font-medium"
                                    :class="parseFloat(row.net_received) > 0 ? 'text-gold-antique' : 'text-taupe'"
        ...
        <div x-show="panel.open" x-transition:enter="transition ease-out duration-300"
            ...
            {{-- Panel Header --}}
            <div class="flex items-center justify-between px-6 py-4 border-b border-champagne flex-shrink-0">
                <div>
                    <h3 class="text-base font-semibold text-brand">
                        Order <span x-text="panel.order?.order_number"></span>
                    </h3>
                    <p class="text-xs text-muted mt-0.5" x-text="panel.order?.customer_name"></p>
                </div>
                <button @click="panel.open = false" class="text-taupe hover:text-muted">
            ...
            <div class="flex-1 overflow-y-auto p-6">
                ...
                <div x-show="!panel.loading">
                    ...
                    <div class="bg-cream rounded-xl p-4 mb-5 grid grid-cols-2 gap-3">
                        <div>
                            <p class="text-xs text-muted mb-0.5">Grand Total</p>
                            <p class="text-base font-bold text-brand" x-text="fmt(panel.order?.grand_total)"></p>
                        </div>
                        <div>
                        ...
                        <div>
                            <p class="text-xs text-muted mb-0.5">Method</p>
                            <p class="text-sm font-medium text-brown uppercase" x-text="panel.order?.payment_method"></p>
                        </div>
                        <div x-show="panel.order?.gateway_transaction_id">
                            <p class="text-xs text-muted mb-0.5">Gateway Ref</p>
                            <p class="text-xs font-mono text-muted truncate" x-text="panel.order?.gateway_transaction_id">
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-5">
                        <div class="bg-ivory rounded-lg p-3 text-center">
                            <p class="text-xs text-primary font-medium">Charged</p>
                            <p class="text-sm font-bold text-gold-antique mt-0.5" x-text="fmt(panel.txSummary?.charged)">
                            </p>
                        </div>
                    ...
                    {{-- Transaction List --}}
                    <h4 class="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Transaction History</h4>
                    <div class="space-y-2 mb-6">
                        <template x-for="tx in panel.transactions" :key="tx.id">
                            <div class="flex items-center justify-between p-3 rounded-lg bg-cream border border-champagne">
                                <div class="flex items-center gap-2">
                                    <span :class="typeColor(tx.type)" class="w-2 h-2 rounded-full flex-shrink-0"></span>
                                    <div>
                                        <p class="text-xs font-medium text-brown capitalize" x-text="tx.type"></p>
                                        <p class="text-xs text-taupe" x-text="tx.description ?? '—'"></p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    <p class="text-xs text-taupe" x-text="fmtDate(tx.created_at)"></p>
                                </div>
                            </div>
                        </template>
                        <p x-show="panel.transactions?.length === 0" class="text-sm text-taupe text-center py-4">
                    ...
                    {{-- Actions --}}
                    @can('order.update')
                        <div class="border-t border-champagne pt-4 space-y-3">
                            {{-- Update Payment Status --}}
                            <div>
                                <h4 class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Update Payment Status
                                </h4>
                                <div class="flex gap-2">
                                    <select x-model="panel.newStatus"
                                        class="flex-1 text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
                                        <option value="unpaid">Unpaid</option>
                                        <option value="paid">Paid</option>
                                        <option value="failed">Failed</option>
                                    </select>
                                    <input x-model="panel.gatewayRef" type="text" placeholder="Gateway ref (optional)"
                                        class="flex-1 text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
                                    <button @click="updatePaymentStatus()" :disabled="panel.updating"
                            ...
                            <div>
                                <h4 class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Add Transaction</h4>
                                ...
                                <div class="flex gap-2 mt-2">
                                    ...
                                    <button @click="addTransaction()"
                                        ...
                                        class="px-4 py-2 bg-primary text-white text-sm rounded-lg hover:bg-gold-antique disabled:opacity-50 transition">
        ...
        {{-- Panel overlay --}}
        <div x-show="panel.open" x-transition.opacity @click="panel.open = false" class="fixed inset-0 bg-black/30 z-40">
        </div>

        {{-- Reconcile Modal (quick fix from reconciliation tab) --}}
        <div x-show="reconcileModal.open" x-transition.opacity
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
            <div @click.outside="reconcileModal.open = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                <h3 class="text-base font-semibold text-brand mb-1">Fix Payment Issue</h3>
                ...
                <div class="space-y-3 mb-5">
                    <div>
                        <label class="text-xs font-medium text-muted block mb-1">Set Payment Status</label>
                        <select x-model="reconcileModal.status"
                            class="w-full text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
                            <option value="unpaid">Unpaid</option>
                            <option value="paid">Paid</option>
                            <option value="failed">Failed</option>
                        </select>
                    ...
                    <div>
                        <label class="text-xs font-medium text-muted block mb-1">Gateway Reference (optional)</label>
                        <input x-model="reconcileModal.gatewayRef" type="text" placeholder="e.g. TXN_123456789"
                            class="w-full text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="text-xs font-medium text-muted block mb-1">Note</label>
                        <input x-model="reconcileModal.note" type="text" placeholder="Reason for this change"
                            class="w-full text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button @click="applyReconcile()" :disabled="reconcileModal.saving"
        ...
        function transactionCenter() {
            return {
                ...
                async loadSummary() {
                    ...
                    try {
                        const r = await fetch('/api/v1/admin/transactions/summary', {
                            headers: this.h()
                        });
                    ...
                    this.chart = new Chart(ctx, {
                        ...
                        options: {
                            ...
                            plugins: {
                                ...
                                tooltip: {
                                    callbacks: {
                                        label: ctx => '৳ ' + parseFloat(ctx.raw).toLocaleString('en', {
                ...
                async loadLedger() {
                    ...
                    try {
                        const p = new URLSearchParams({
                            ...
                            ...(this.ledgerFrom && {
                                from: this.ledgerFrom
                            }),
                            ...(this.ledgerTo && {
                                to: this.ledgerTo
                            }),
                        });
                        const r = await fetch(`/api/v1/admin/transactions?${p}`, {
                            headers: this.h()
                        });
                        const d = await r.json();
                        if (d.success) {
                ...
                async loadReconciliation() {
                    ...
                    try {
                        const p = new URLSearchParams({
                            page: this.reconPage,
                            ...(this.reconIssue && {
                                issue_type: this.reconIssue
                            }),
                            ...(this.reconMethod && {
                                method: this.reconMethod
                            }),
                        });
                        const r = await fetch(`/api/v1/admin/transactions/reconciliation?${p}`, {
                            headers: this.h()
                        });
                        const d = await r.json();
                        if (d.success) {
                ...
                async applyReconcile() {
                    ...
                    try {
                        const r = await fetch(
                            `/api/v1/admin/transactions/order/${this.reconcileModal.row.id}/payment-status`, {
                                method: 'PATCH',
                                headers: {
                                    ...this.h(),
                        ...
                        this.reconcileModal.saving = false;
                    }
                },
                ...
                async openOrderPanel(orderId) {
                    ...
                    try {
                        const r = await fetch(`/api/v1/admin/transactions/order/${orderId}`, {
                            headers: this.h()
                        });
                        const d = await r.json();
                        if (d.success) {
                ...
                async addTransaction() {
                    ...
                    try {
                        ...
                        if (r.ok && d.success) {
                            ...
                            this.panel.txError = d.message || (d.errors ? Object.values(d.errors).flat().join(', ') :
                                'Failed');
                        }
                    } catch (e) {
                        this.panel.txError = 'Network error';
                    } finally {
                        this.panel.addingTx = false;
                    }
                },
                ...
                typeBadge(type) {
                    const map = {
                        charge: 'bg-cream text-gold-antique',
                        refund: 'bg-red-100 text-red-700',
                        discount: 'bg-amber-100 text-amber-700',
                        coupon: 'bg-purple-100 text-purple-700',
                        shipping: 'bg-blue-100 text-blue-700',
                        commission: 'bg-gray-100 text-brown',
                    };
                    return map[type] ?? 'bg-gray-100 text-brown';
                },
