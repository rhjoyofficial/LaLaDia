    ...
    <div class="border-b border-champagne mb-6">
        <nav class="-mb-px flex gap-6">
            <button @click="switchTab('summary')"
                    :class="tab === 'summary' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-muted hover:text-brown'"
            ...
            <button @click="switchTab('ledger')"
                    :class="tab === 'ledger' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-muted hover:text-brown'"
                    class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
            ...
            <button @click="switchTab('reconciliation')"
                    :class="tab === 'reconciliation' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-muted hover:text-brown'"
                    class="inline-flex items-center gap-1.5 pb-3 text-sm font-medium border-b-2 transition">
        ...
        <div x-show="!summaryLoading && summary">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne cursor-pointer hover:border-red-300 transition"
                    ...
                    <p class="text-2xl font-bold"
                       :class="summary.discrepancies > 0 ? 'text-red-600' : 'text-primary'"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-champagne">
                    ...
                    <div x-show="(summary.by_method ?? []).length === 0"
                         class="text-sm text-taupe text-center py-8">No data</div>
                    <template x-for="m in (summary.by_method ?? [])" :key="m.method">
                        <div class="mb-4">
                            <div class="flex justify-between text-sm mb-1">
                                <span class="font-medium text-brown capitalize" x-text="m.method === 'cod' ? 'Cash on Delivery' : 'SSLCommerz'"></span>
    ...
    <div x-show="tab === 'ledger'">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-champagne p-4 mb-4 flex flex-wrap gap-3 items-center">
            ...
            <input x-model="ledgerFrom" @change="ledgerPage=1; loadLedger()"
                   type="date" class="text-sm border border-champagne rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500"
                   title="From date">
            ...
            <table x-show="!ledgerLoading" class="min-w-full divide-y divide-gray-100">
                ...
                <tbody class="divide-y divide-gray-100">
                    <template x-for="tx in ledger" :key="tx.id">
                        <tr class="hover:bg-cream transition cursor-pointer"
                            ...
                            <td class="px-5 py-3.5 text-sm text-muted max-w-xs truncate" x-text="tx.description ?? '—'"></td>
    ...
    <div x-show="tab === 'reconciliation'">
        ...
        <div x-show="reconCounts" class="flex flex-wrap gap-2 mb-4">
            <button @click="reconIssue=''; reconPage=1; loadReconciliation()"
                    :class="reconIssue === '' ? 'bg-gray-800 text-white' : 'bg-white text-muted border border-champagne'"
                ...
                <tbody class="divide-y divide-gray-100">
                    <template x-for="row in reconRows" :key="row.id">
                        <tr class="hover:bg-cream transition">
                            ...
                            <td class="px-5 py-3.5">
                                <span class="text-xs bg-gray-100 text-muted px-2 py-0.5 rounded-full font-medium uppercase"
                            ...
                            <td class="px-5 py-3.5 text-right text-sm"
                                :class="parseFloat(row.charged) >= parseFloat(row.grand_total) ? 'text-gold-antique' : 'text-red-600'"
            ...
            <div x-show="!panel.loading">
                ...
                <div class="bg-cream rounded-xl p-4 mb-5 grid grid-cols-2 gap-3">
                    ...
                    <div x-show="panel.order?.gateway_transaction_id">
                        ...
                        <p class="text-xs font-mono text-muted truncate" x-text="panel.order?.gateway_transaction_id"></p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2 mb-5">
                    <div class="bg-ivory rounded-lg p-3 text-center">
                        ...
                        <p class="text-sm font-bold text-gold-antique mt-0.5" x-text="fmt(panel.txSummary?.charged)"></p>
                    </div>
                ...
                <div class="border-t border-champagne pt-4 space-y-3">
                    ...
                    <div>
                        <h4 class="text-xs font-semibold text-muted uppercase tracking-wide mb-2">Update Payment Status</h4>
                        <div class="flex gap-2">
...
function transactionCenter() {
    return {
        ...
        typeColor(type) {
            const map = {
                charge: 'bg-ivory0', refund: 'bg-red-500', discount: 'bg-amber-500',
