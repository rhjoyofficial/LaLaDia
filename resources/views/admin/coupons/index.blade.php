                ...
                <table class="w-full text-sm">
                    <thead class="bg-cream border-b border-champagne">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">Total Saved</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-cream transition">
                                    <td class="px-5 py-3">
                                        <div class="flex items-center gap-2">
                                            <span class="font-mono text-xs font-semibold text-brown tracking-wide"
                                                x-text="c.code"></span>
                                            <button @click="copyCode(c.code)"
                                                class="text-taupe hover:text-gold-antique transition cursor-pointer text-xs"
                                                title="Copy">
                                    ...
                                    <td class="px-5 py-3 text-xs font-semibold text-indigo-700">
                                        ...
                                        <span x-show="c.total_discount == null" class="text-taupe">—</span>
                                    </td>
                                    <td class="px-5 py-3 text-xs text-muted">
                        ...
                        <template x-if="!loading && coupons.length === 0">
                            <tr>
                                <td colspan="8" class="px-5 py-12 text-center text-taupe">
            ...
            <div @click.outside="showFormModal = false"
                ...
                <form @submit.prevent="saveCoupon()" class="px-6 py-5 space-y-4">
                    ...
                    <div x-show="!editingId">
                        <label class="block text-xs font-semibold text-muted mb-1">Code <span
                                class="text-red-500">*</span></label>
                        <input type="text" x-model="form.code"
                            :class="errors.code ? 'border-red-400' : 'border-champagne'" placeholder="e.g. SAVE20"
                    ...
                    <div x-show="editingId">
                        ...
                        <p class="font-mono font-bold text-brown text-sm tracking-wide px-3 py-2 bg-cream rounded-lg"
                            x-text="form.code"></p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1">Type <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1">
                                ...
                                <span class="text-taupe font-normal"
                    ...
                    {{-- Limit per user --}}
                    <div>
                        <label class="block text-xs font-semibold text-muted mb-1">Limit Per User</label>
        ...
        <div x-show="showBulkModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
            ...
            <div @click.outside="showBulkModal = false"
                    ...
                    <div class="px-6 py-4 border-b border-champagne bg-ivory">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button @click="copyAllCodes()"
                                class="text-xs text-gold-antique hover:underline cursor-pointer font-medium">
                                Copy all
                            </button>
                        </div>
                        <div class="max-h-40 overflow-y-auto rounded-lg bg-white border border-sand p-3">
                            <div class="grid grid-cols-2 gap-1">
                                <template x-for="code in generatedCodes" :key="code">
                                    <span class="font-mono text-xs text-brown bg-cream px-2 py-0.5 rounded"
                                        x-text="code"></span>
                                </template>
                            </div>
                        </div>
                        <button @click="generatedCodes = []; resetBulkForm()"
                            class="mt-3 text-xs text-gold-antique hover:underline cursor-pointer">
                            Generate more
                        </button>
                    </div>
                </template>
                ...
                <form x-show="generatedCodes.length === 0" @submit.prevent="doBulkGenerate()"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1">Prefix <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1">Count <span
                                    class="text-red-500">*</span> <span class="text-taupe font-normal">(max
                                    500)</span></label>
                            <input type="number" x-model="bulkForm.count" min="1" max="500"
                                placeholder="10" :class="bulkErrors.count ? 'border-red-400' : 'border-champagne'"
                                class="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gold-antique">
                            <p x-show="bulkErrors.count" class="text-xs text-red-500 mt-1"
                                x-text="bulkErrors.count?.[0]"></p>
                        </div>
                    </div>
                    <p class="text-xs text-taupe -mt-2">Codes will look like: <span
                            class="font-mono font-semibold text-muted"
