...
@section('content')
    <div x-data="webhooksApp()" x-init="init()" x-cloak>
            ...
            <template x-for="ev in allowedEvents" :key="ev">
                <div class="bg-white border border-champagne rounded-xl px-3 py-2.5 flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full shrink-0"
                        :class="webhooks.some(w => w.event === ev && w.is_active) ? 'bg-ivory' : 'bg-gray-300'"></span>
        ...
        <div x-show="!loading" class="bg-white rounded-xl border border-champagne overflow-hidden">
                    ...
                    <template x-for="wh in webhooks" :key="wh.id">
                        <tr class="hover:bg-cream transition">
                            <td class="px-5 py-3.5">
                                <span
                                    class="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-mono font-medium"
                                    :class="eventColors[wh.event] ?? 'bg-gray-100 text-brown'" x-text="wh.event"></span>
                            </td>
                            <td class="px-5 py-3.5 max-w-xs">
                                <span class="text-sm text-brown truncate block" x-text="wh.url"
                                    :title="wh.url"></span>
                            </td>
                            <td class="px-5 py-3.5">
                            ...
                            <td class="px-5 py-3.5">
                                <button @click="toggleActive(wh)"
                                    ...
                                    <span class="w-1.5 h-1.5 rounded-full"
                                        :class="wh.is_active ? 'bg-ivory' : 'bg-gray-400'"></span>
                            ...
                            <td class="px-5 py-3.5 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button @click="testWebhook(wh)" :disabled="!wh.is_active || testing[wh.id]"
                                        :class="(!wh.is_active || testing[wh.id]) ? 'opacity-40 cursor-not-allowed' :
                                        'hover:text-indigo-600 hover:bg-indigo-50 cursor-pointer'"
                                        class="p-1.5 text-taupe rounded transition" title="Send test ping">
                                        <i class="fas text-xs"
                                            :class="testing[wh.id] ? 'fa-circle-notch fa-spin' : 'fa-paper-plane'"></i>
                                    </button>
            ...
            {{-- Empty state --}}
            <div x-show="webhooks.length === 0" class="flex flex-col items-center justify-center py-20 text-center px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-indigo-700">
                ...
                <div class="flex gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-indigo-500">Failed deliveries are retried up to 3 times with exponential backoff (30s
                            → 120s → 300s) via the queue system.</p>
                    </div>
                </div>
                <div class="flex gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-indigo-500">Webhooks are dispatched on a background queue — they never delay your
                            store's HTTP responses regardless of your receiver's speed.</p>
                    </div>
                </div>
            </div>
  ...
  "data": { ... event-specific fields ... }
        ...
        <div x-show="createModal.open" x-transition.opacity
            ...
            <div @click.outside="createModal.open = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">

                <h3 class="text-base font-semibold text-brand mb-1">Register Webhook</h3>
                <p class="text-xs text-muted mb-5">Your URL will receive a signed POST request when the selected event
                    fires.</p>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input x-model="createModal.secret" :type="createModal.showSecret ? 'text' : 'password'"
                            ...
                            <button type="button" @click="createModal.showSecret = !createModal.showSecret"
                ...
                <div class="flex justify-end gap-2 mt-6">
                    ...
                    <button @click="createWebhook()" :disabled="createModal.loading"
                        ...
                        <i class="fas mr-1" :class="createModal.loading ? 'fa-circle-notch fa-spin' : 'fa-plus'"></i>
        ...
        function webhooksApp() {
            return {
                ...
                toast(msg, type = 'success') {
                    ...
                    el.className =
                        `fixed bottom-5 right-5 z-[9999] px-4 py-3 rounded-lg shadow-lg text-sm font-medium flex items-center gap-2 ${type === 'success' ? 'bg-primary text-white' : 'bg-red-600 text-white'}`;
                    el.innerHTML =
