        ...
        <div x-show="!loading && order">
            ...
            <div class="flex items-start justify-between mb-6">
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.orders') }}" class="text-taupe hover:text-brown transition">
                        <i class="fa-solid fa-arrow-left"></i>
                    </a>
                    <div>
                        <div class="flex items-center gap-3">
                            <h2 class="text-lg font-bold text-brown font-mono"
                ...
                @can('order.update')
                    <div class="flex items-center gap-2">
                        ...
                        <button x-show="editMode" @click="cancelEdit()"
                            class="inline-flex items-center gap-2 px-3 py-2 bg-gray-100 text-brown text-sm font-medium rounded-lg hover:bg-gray-200 transition cursor-pointer">
            ...
            ============================= --}}
                ...
                ═══════════════════════════════ --}}
                    <div x-show="!editMode" class="bg-white border border-champagne rounded-xl overflow-hidden">
                        <div class="px-5 py-4 border-b border-champagne">
                            <h3 class="text-sm font-bold text-brown">Order Items</h3>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead class="bg-cream">
                                    <tr>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">
                                            Product</th>
                                        <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">SKU
                                        </th>
                                        <th class="px-5 py-3 text-center text-xs font-semibold text-muted uppercase">Qty
                                        </th>
                                        <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">Unit
                                            Price</th>
                                        <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">Total
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50">
                                    <template x-for="(item, i) in order?.items ?? []" :key="i">
                                        <tr>
                                            <td class="px-5 py-3">
                                                <p class="font-medium text-brown" x-text="item.product_name"></p>
                                                <p class="text-xs text-taupe" x-text="item.variant_title"></p>
                                            </td>
                                            <td class="px-5 py-3 font-mono text-xs text-muted" x-text="item.sku ?? '—'">
                                            </td>
                                            <td class="px-5 py-3 text-center text-brown" x-text="item.qty"></td>
                                            <td class="px-5 py-3 text-right text-brown">
                                                ৳<span x-text="Number(item.unit_price).toLocaleString()"></span>
                                            </td>
                                            <td class="px-5 py-3 text-right font-semibold text-brand">
                                                ৳<span x-text="Number(item.total).toLocaleString()"></span>
                                            </td>
                                        </tr>
                                    </template>
                                </tbody>
                                <tfoot class="bg-cream border-t border-champagne">
                                    <tr>
                                        <td colspan="4" class="px-5 py-2 text-right text-xs text-muted">Subtotal</td>
                                        <td class="px-5 py-2 text-right text-sm text-brown">৳<span
                                                x-text="Number(order?.subtotal ?? 0).toLocaleString()"></span></td>
                                    </tr>
                                    <template x-if="order?.discount_total > 0">
                                        <tr>
                                            <td colspan="4" class="px-5 py-1 text-right text-xs text-gold-antique">
                                                Discount <span x-show="order?.coupon_code"
                                                    x-text="'(' + order.coupon_code + ')'"></span>
                                            </td>
                                            <td class="px-5 py-1 text-right text-sm text-gold-antique">−৳<span
                                                    x-text="Number(order?.discount_total ?? 0).toLocaleString()"></span>
                                            </td>
                                        </tr>
                                    </template>
                                    <tr>
                                        <td colspan="4" class="px-5 py-1 text-right text-xs text-muted">Shipping</td>
                                        <td class="px-5 py-1 text-right text-sm text-brown">৳<span
                                                x-text="Number(order?.shipping_cost ?? 0).toLocaleString()"></span></td>
                                    </tr>
                                    <tr class="border-t border-champagne">
                                        <td colspan="4" class="px-5 py-2.5 text-right text-sm font-bold text-brown">
                                            Grand Total</td>
                                        <td class="px-5 py-2.5 text-right text-base font-bold text-brand">৳<span
                ...
                ═══════════════════════════════ --}}
                    <div x-show="editMode" x-cloak class="space-y-4">
                        ...
                        <div class="bg-white border border-champagne rounded-xl">
                            <div class="px-5 py-3 border-b border-champagne">
                                <h4 class="text-sm font-bold text-brown"><i
                                        class="fas fa-user mr-2 text-taupe"></i>Customer Information</h4>
                            </div>
                            <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-muted mb-1.5">Phone</label>
                                ...
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-semibold text-muted mb-1.5">Address Line</label>
                                    <textarea x-model="editCustomer.address_line" rows="2"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-muted mb-1.5">Area / Thana</label>
                                    <input type="text" x-model="editCustomer.area"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-muted mb-1.5">City</label>
                                    <input type="text" x-model="editCustomer.city"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-muted mb-1.5">Postal Code</label>
                                    <input type="text" x-model="editCustomer.postal_code"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-muted mb-1.5">Shipping Zone</label>
                                ...
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-semibold text-muted mb-1.5">Order Notes</label>
                        ...
                        {{-- Add Product Search --}}
                        <div class="bg-white border border-champagne rounded-xl p-4">
                            <label class="block text-xs font-medium text-muted mb-2">Add Product</label>
                            <div class="relative">
                                ...
                                <i class="fas fa-search absolute right-3 top-3 text-taupe text-sm"></i>
                            </div>

                            {{-- Search Results Dropdown --}}
                            <div x-show="searchResults.length > 0" x-cloak
                                class="mt-2 border border-champagne rounded-lg divide-y divide-gray-100 max-h-60 overflow-y-auto bg-white shadow-lg">
                                <template x-for="result in searchResults"
                                    ...
                                    <button @click="addItemFromSearch(result)"
                                        ...
                                        <div class="text-right shrink-0">
                                            <p class="text-sm font-semibold text-brown">৳<span
                                                    x-text="Number(result.price).toLocaleString()"></span></p>
                                            <p class="text-xs"
                                                :class="result.available_stock > 0 ? 'text-primary' : 'text-red-500'"
                        ...
                        <div class="bg-white border border-champagne rounded-xl overflow-hidden">
                            <div class="px-5 py-4 border-b border-champagne flex items-center justify-between">
                                <h3 class="text-sm font-bold text-brown">Order Items (Editing)</h3>
                                <span class="text-xs text-taupe" x-text="editItems.length + ' item(s)'"></span>
                            </div>
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    <thead class="bg-cream">
                                        <tr>
                                            <th class="px-5 py-3 text-left text-xs font-semibold text-muted uppercase">
                                                Product</th>
                                            <th
                                                class="px-5 py-3 text-center text-xs font-semibold text-muted uppercase w-32">
                                                Qty</th>
                                            <th class="px-5 py-3 text-right text-xs font-semibold text-muted uppercase">
                                                Unit Price</th>
                                            <th
                                                class="px-5 py-3 text-center text-xs font-semibold text-muted uppercase w-16">
                                                Remove</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-50">
                                            ...
                                            <tr :class="item._removed ? 'bg-red-50 opacity-50' : ''">
                                                <td class="px-5 py-3">
                                                    ...
                                                    <p x-show="item.sku_snapshot || item.sku"
                                                        class="text-xs text-taupe font-mono"
                                                ...
                                                <td class="px-5 py-3 text-center">
                                                    <div class="flex items-center justify-center gap-1">
                                                        ...
                                                        <input type="number" x-model.number="item.quantity"
                                                            ...
                                                            class="w-14 text-center border border-champagne rounded py-1 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                        ...
                                        <template x-if="editItems.length === 0">
                                            <tr>
                                                <td colspan="4" class="px-5 py-8 text-center text-taupe text-sm">
                        ...
                        <div class="bg-white border border-champagne rounded-xl p-4" x-show="nextStatuses.length > 0">
                            <h4 class="text-xs font-semibold text-muted uppercase mb-3"><i class="fas fa-arrows-rotate mr-1 text-taupe"></i>Update Order Status</h4>
                            <div class="flex gap-2">
                                <select x-model="newStatus"
                                    class="flex-1 border border-champagne rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gold-antique cursor-pointer">
                                ...
                                <button x-show="newStatus" @click="updateStatus()" :disabled="changingStatus"
                                    class="px-4 py-2 bg-gold-antique hover:bg-gold-antique disabled:opacity-50 text-white text-sm font-medium rounded-lg transition cursor-pointer">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button x-show="previewData" @click="applyEdit()" :disabled="applying"
                                class="inline-flex items-center gap-2 px-4 py-2.5 bg-primary text-white text-sm font-medium rounded-lg hover:bg-gold-antique disabled:opacity-50 transition">
                        ...
                        <template x-if="previewData">
                            <div class="bg-white border border-sand rounded-xl overflow-hidden">
                                <div class="px-5 py-3 bg-ivory border-b border-champagne">
                                    <h4 class="text-sm font-bold text-gold-antique"><i class="fas fa-calculator mr-1"></i>
                                ...
                                <div class="p-5 space-y-2 text-sm">
                                    ...
                                    <div x-show="previewData.tier_discount > 0"
                                        class="flex justify-between text-gold-antique">
                                    ...
                                    <div class="flex justify-between border-t border-champagne pt-2">
                                        <span class="font-bold text-brown">Grand Total</span>
                                        <span class="font-bold text-lg text-brand">৳<span
                                ...
                                <template x-if="previewData.changes && previewData.changes.length > 0">
                                    <div class="px-5 pb-4">
                                        <p class="text-xs font-semibold text-muted uppercase mb-2">Changes</p>
                                        <div class="space-y-1">
                                            <template x-for="change in previewData.changes"
                                                ...
                                                <div class="flex items-center gap-2 text-xs px-2 py-1 rounded"
                                                    :class="{
                                                        ...
                                                        'bg-ivory text-gold-antique': change.type === 'added',
                ...
                ═══════════════════════════════ --}}
                    @can('order.update')
                        <div class="bg-white border border-champagne rounded-xl overflow-hidden">
                            <div class="px-5 py-4 border-b border-champagne flex items-center justify-between">
                                <h3 class="text-sm font-bold text-brown">
                                    <i class="fas fa-truck-fast text-primary mr-1"></i> Courier & Shipments
                                </h3>
                                <span class="text-xs px-2 py-0.5 rounded-full"
                                    :class="(order?.shipments?.length ?? 0) > 0 ? 'bg-cream text-gold-antique' :
                                        'bg-gray-100 text-muted'"
                                    x-text="(order?.shipments?.length ?? 0) + ' shipment(s)'"></span>
                            </div>

                            {{-- Assign Courier Buttons (only when confirmed or processing) --}}
                            <div class="p-5 border-b border-champagne"
                                 x-show="['confirmed', 'processing'].includes(order?.order_status)" x-cloak>
                                <p class="text-xs font-semibold text-muted uppercase mb-3">Assign Courier</p>
                                <div class="grid grid-cols-2 gap-2">
                                    <button @click="openPathaoModal()"
                                        class="inline-flex items-center justify-center gap-2 px-3 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-gold-antique transition cursor-pointer">
                            ...
                            <div x-show="showPathaoModal" x-cloak
                                ...
                                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" @click.stop>
                                    <div class="px-6 py-4 border-b border-champagne flex items-center justify-between">
                                        ...
                                        <button @click="showPathaoModal = false" class="text-taupe hover:text-muted cursor-pointer"><i class="fas fa-xmark text-lg"></i></button>
                                    </div>
                                    ...
                                    <div class="p-6 space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <textarea x-model="pathao.shipping_note" rows="2"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:outline-none resize-none"
                                    ...
                                    <div class="px-6 py-4 border-t border-champagne flex justify-end gap-3">
                            ...
                            <div x-show="showSimpleCourierModal" x-cloak
                                ...
                                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md" @click.stop>
                                    <div class="px-6 py-4 border-b border-champagne flex items-center justify-between">
                                        <h2 class="text-base font-bold text-brown capitalize">
                                            <i class="fas fa-truck text-blue-600 mr-2"></i>
                                            Assign to <span x-text="simpleCourier.label"></span>
                                        </h2>
                                        <button @click="showSimpleCourierModal = false" class="text-taupe hover:text-muted cursor-pointer"><i class="fas fa-xmark text-lg"></i></button>
                                    </div>
                                    <div class="p-6 space-y-4">
                                        <div>
                                            <label class="block text-xs font-semibold text-muted mb-1">Shipping Address <span class="text-red-500">*</span></label>
                            ...
                            <div class="divide-y divide-gray-100">
                                    ...
                                    <div class="p-5">
                                        <div class="flex items-start justify-between mb-3">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <button @click="syncShipmentStatus(shipment.id)"
                                                    ...
                                                    class="text-xs px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 text-muted disabled:opacity-30 transition cursor-pointer"
                                        ...
                                        <div class="grid grid-cols-2 gap-3 text-xs">
                                            <div>
                                                ...
                                                <p class="font-mono font-medium text-brown"
                                            ...
                                            <div x-show="shipment.delivery_fee !== null">
                                                ...
                                                <p class="font-medium text-brown">৳<span
                                        ...
                                        <div class="mt-3 flex items-center justify-between text-xs text-taupe">
                                        ...
                                        <div class="mt-1 text-xs text-taupe" x-show="shipment.created_by_name">
                                            Created by <span class="font-medium text-muted"
                                ...
                                <template x-if="(order?.shipments ?? []).length === 0">
                                    <div class="p-5 text-center text-sm text-taupe">
                    ...
                    <div class="bg-white border border-champagne rounded-xl p-5">
                        <h3 class="text-sm font-bold text-brown mb-4">Order Timeline</h3>
                        <div class="relative">
                            ...
                            <div class="space-y-4">
                                <template x-for="(event, i) in order?.timeline ?? []" :key="i">
                                    <div class="flex gap-4 items-start">
                                        ...
                                        <div class="pt-1">
                                            <p class="text-sm font-semibold text-brown" x-text="event.label"></p>
                                            <p class="text-xs text-taupe"
                                ...
                                <template x-if="!order?.timeline?.length">
                                    <p class="text-sm text-taupe pl-12">No timeline events yet</p>
                                </template>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Admin Notes --}}
                    <div class="bg-white border border-champagne rounded-xl p-5">
                        <h3 class="text-sm font-bold text-brown mb-4">Internal Notes</h3>
                        ...
                        <div class="space-y-3 mb-4">
                            <template x-for="note in order?.admin_notes ?? []" :key="note.id">
                                <div class="bg-yellow-50 border border-yellow-100 rounded-lg px-4 py-3">
                                    <p class="text-sm text-brown" x-text="note.body"></p>
                            ...
                            <template x-if="!order?.admin_notes?.length">
                                <p class="text-sm text-taupe">No internal notes yet.</p>
                            </template>
                        </div>

                        @can('order.update')
                            <div class="flex gap-3 mt-4 border-t border-champagne pt-4">
                                <textarea x-model="noteBody" rows="2" placeholder="Add a note visible only to admins…"
                                    class="flex-1 border border-champagne rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gold-antique resize-none"
                                    @keydown.meta.enter="addNote()"></textarea>
                                <button @click="addNote()" :disabled="addingNote || !noteBody.trim()"
                                    class="self-end px-4 py-2 bg-gold-antique hover:bg-gold-antique disabled:opacity-50 text-white text-sm font-medium rounded-lg transition cursor-pointer">
            ...
            ============================= --}}
                <div class="space-y-5">
                    ...
                    @can('order.update')
                        <div class="bg-white border border-champagne rounded-xl p-5">
                            <h3 class="text-sm font-bold text-brown mb-3">Update Status</h3>

                            <div x-show="nextStatuses.length === 0" class="text-sm text-taupe italic">
                            ...
                            <div x-show="nextStatuses.length > 0" class="space-y-3">
                                ...
                                <button @click="updateStatus()" :disabled="!newStatus || changingStatus"
                                    class="w-full inline-flex items-center justify-center gap-2 bg-gold-antique hover:bg-gold-antique disabled:opacity-50 text-white text-sm font-medium py-2 rounded-lg transition cursor-pointer">
                    ...
                    {{-- Customer Info --}}
                    <div class="bg-white border border-champagne rounded-xl p-5">
                        <h3 class="text-sm font-bold text-brown mb-3">Customer</h3>
                        <div class="space-y-2 text-sm">
                            <div class="flex items-start gap-2">
                                <i class="fa-solid fa-user text-taupe text-xs mt-1 w-4 text-center"></i>
                                <span class="text-brown" x-text="order?.customer_name ?? '—'"></span>
                            </div>
                            <div class="flex items-start gap-2">
                                <i class="fa-solid fa-phone text-taupe text-xs mt-1 w-4 text-center"></i>
                                <span class="text-brown" x-text="order?.customer_phone ?? '—'"></span>
                            </div>
                            <template x-if="order?.customer_email">
                                <div class="flex items-start gap-2">
                                    <i class="fa-solid fa-envelope text-taupe text-xs mt-1 w-4 text-center"></i>
                                    <span class="text-brown break-all" x-text="order.customer_email"></span>
                                </div>
                            </template>
                            <template x-if="order?.user?.id">
                                <div class="pt-2 border-t border-champagne">
                    ...
                    {{-- Shipping Address --}}
                    <div class="bg-white border border-champagne rounded-xl p-5">
                        <h3 class="text-sm font-bold text-brown mb-3">Delivery Address</h3>
                        <template x-if="order?.shipping_address">
                            <div class="text-sm text-muted space-y-1">
                                ...
                                <p class="text-xs text-taupe mt-1" x-show="order.zone">Zone: <span
                                        x-text="order.zone?.name"></span></p>
                            </div>
                        </template>
                        <template x-if="!order?.shipping_address">
                            <p class="text-sm text-taupe">No address recorded.</p>
                        </template>
                    </div>
                    ...
                    <div class="bg-white border border-champagne rounded-xl p-5">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-sm font-bold text-brown">Payment</h3>
                            @can('order.update')
                            <button x-show="order?.payment_status !== 'paid'" @click="markAsPaid()"
                                :disabled="markingPaid"
                                class="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold bg-cream text-gold-antique hover:bg-champagne rounded-lg transition disabled:opacity-50 cursor-pointer">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="uppercase font-medium text-brown"
                            ...
                            <template x-if="order?.coupon_code">
                                <div class="flex justify-between pt-2 border-t border-champagne">
                                    <span class="text-muted">Coupon</span>
                                    <span class="font-mono text-xs font-medium text-gold-antique"
                                        x-text="order.coupon_code"></span>
                                </div>
                            </template>
                            <template x-if="order?.notes">
                                <div class="pt-2 border-t border-champagne">
                                    <p class="text-xs text-muted mb-1">Customer note:</p>
                                    <p class="text-xs text-brown italic" x-text="order.notes"></p>
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    <template x-if="(order?.shipments ?? []).length > 0">
                        <div class="bg-white border border-champagne rounded-xl p-5">
                            <h3 class="text-sm font-bold text-brown mb-3">Latest Shipment</h3>
                            <div class="space-y-2 text-sm" x-data="{ s: order.shipments[0] }">
                                <div class="flex justify-between">
                                    <span class="text-muted">Courier</span>
                                    <span class="font-medium text-brown" x-text="s.courier_label"></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-muted">Status</span>
                                ...
                                <div class="flex justify-between">
                                    <span class="text-muted">Tracking</span>
                                    <span class="font-mono text-xs font-medium text-brown"
