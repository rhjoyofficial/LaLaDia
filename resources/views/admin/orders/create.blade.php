...
<div x-data="createOrder()" x-init="init()" x-cloak>
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h2 class="text-lg font-bold text-brown">Create New Order</h2>
            <p class="text-sm text-muted">Manual order creation from admin panel</p>
        </div>
    </div>

    {{-- Success Banner --}}
    <div x-show="createdOrder" x-transition class="mb-6 bg-ivory border border-sand rounded-xl p-5 flex items-start gap-4">
        <i class="fas fa-circle-check text-ivory0 text-xl mt-0.5"></i>
        <div>
            <p class="font-semibold text-gold-antique">Order created successfully!</p>
            <p class="text-sm text-gold-antique mt-1">
            ...
            <div class="mt-3 flex gap-3">
                <a :href="`/admin/orders/${createdOrder?.id}`"
                   class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-gold-antique transition">
                    <i class="fas fa-eye"></i> View Order
                </a>
                <button @click="resetForm()"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-brown text-sm font-medium rounded-lg hover:bg-cream transition">
    ...
    <div x-show="!createdOrder">
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 space-y-6">
                ...
                <div class="bg-white border border-champagne rounded-xl">
                    <div class="px-5 py-4 border-b border-champagne">
                        <h3 class="text-sm font-bold text-brown"><i class="fas fa-user mr-2 text-taupe"></i>Customer Information</h3>
                    </div>
                    <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Full Name <span class="text-red-500">*</span></label>
                            <input type="text" x-model="form.customer_name"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Customer full name">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Phone Number <span class="text-red-500">*</span></label>
                            <input type="text" x-model="form.customer_phone"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="01XXXXXXXXX">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Email Address</label>
                            <input type="email" x-model="form.customer_email"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Optional">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Link to Registered Customer</label>
                            <input type="text" x-model="userSearch"
                                   @input.debounce.400ms="searchUsers()"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Search by phone / name…">
                            <div x-show="userResults.length" class="mt-1 bg-white border border-champagne rounded-lg shadow-lg max-h-40 overflow-y-auto z-30 relative">
                                <template x-for="u in userResults" :key="u.id">
                                    <div @click="selectUser(u)" class="px-3 py-2 text-sm cursor-pointer hover:bg-blue-50 flex items-center gap-2">
                                        <i class="fas fa-user text-taupe text-xs"></i>
                                        <span x-text="u.name"></span>
                                        <span class="text-taupe text-xs" x-text="u.phone"></span>
                                    </div>
                                </template>
                            </div>
                            <p x-show="form.linked_user_id" class="text-xs text-primary mt-1">
                ...
                <div class="bg-white border border-champagne rounded-xl">
                    <div class="px-5 py-4 border-b border-champagne">
                        <h3 class="text-sm font-bold text-brown"><i class="fas fa-map-marker-alt mr-2 text-taupe"></i>Shipping Address</h3>
                    </div>
                    <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            <label class="block text-xs font-semibold text-muted mb-1.5">Address Line <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">City <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Shipping Zone <span class="text-red-500">*</span></label>
                ...
                <div class="bg-white border border-champagne rounded-xl">
                    <div class="px-5 py-4 border-b border-champagne flex items-center justify-between">
                        <h3 class="text-sm font-bold text-brown"><i class="fas fa-box mr-2 text-taupe"></i>Order Items</h3>
                        <span class="text-xs text-taupe" x-text="items.length + ' item(s)'"></span>
                    </div>

                    {{-- Product search --}}
                    <div class="p-5 border-b border-champagne">
                        <label class="block text-xs font-semibold text-muted mb-1.5">Search &amp; Add Product</label>
                        <div class="relative">
                            ...
                            <i class="fas fa-search absolute left-3 top-2.5 text-taupe text-xs"></i>
                        </div>
                        <div x-show="searchResults.length" class="mt-1 bg-white border border-champagne rounded-lg shadow-lg max-h-56 overflow-y-auto z-30 relative">
                            <template x-for="result in searchResults" :key="(result.variant_id ?? 'c'+result.combo_id)">
                                <div @click="addItem(result)"
                                    ...
                                    <img :src="result.thumbnail || '/images/placeholder.png'" class="w-9 h-9 rounded-lg object-cover border border-champagne">
                                    <div class="flex-1 min-w-0">
                                        <p class="font-semibold text-brown truncate" x-text="result.product_name"></p>
                                        <p class="text-xs text-muted" x-text="result.variant_title + (result.sku ? ' · ' + result.sku : '')"></p>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold text-brown" x-text="'৳' + result.price.toLocaleString()"></p>
                                        <p class="text-xs" :class="result.available_stock > 0 ? 'text-primary' : 'text-red-500'"
                    ...
                    <div class="divide-y divide-gray-50">
                        <template x-for="(item, idx) in items" :key="idx">
                            <div class="px-5 py-4 flex items-center gap-4">
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-brown truncate" x-text="item.product_name"></p>
                                    <p class="text-xs text-muted" x-text="item.variant_title + (item.sku ? ' · ' + item.sku : '')"></p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <button @click="changeQty(idx, -1)"
                                            class="w-7 h-7 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-muted text-sm transition">
                                ...
                                <div class="w-24 text-right">
                                    <p class="text-sm font-bold text-brown" x-text="'৳' + (item.price * item.quantity).toLocaleString()"></p>
                                    <p class="text-xs text-taupe" x-text="'৳' + item.price.toLocaleString() + ' ea'"></p>
                                </div>
                        ...
                        <div x-show="items.length === 0" class="px-5 py-10 text-center text-taupe">
                ...
                <div class="bg-white border border-champagne rounded-xl">
                    <div class="px-5 py-4 border-b border-champagne">
                        <h3 class="text-sm font-bold text-brown"><i class="fas fa-note-sticky mr-2 text-taupe"></i>Order Notes</h3>
                    </div>
                    <div class="p-5">
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white border border-champagne rounded-xl">
                    <div class="px-5 py-4 border-b border-champagne">
                        <h3 class="text-sm font-bold text-brown"><i class="fas fa-credit-card mr-2 text-taupe"></i>Payment</h3>
                    </div>
                    <div class="p-5 space-y-3">
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Payment Method <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Payment Status</label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-muted mb-1.5">Coupon Code</label>
                            <div class="flex gap-2">
                                <input type="text" x-model="form.coupon_code"
                                       class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                       placeholder="Optional">
                                <button @click="recalculate()"
                                        class="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-muted text-sm rounded-lg transition">
                                    Apply
                                </button>
                            </div>
                        </div>
                ...
                <div class="bg-white border border-champagne rounded-xl">
                    <div class="px-5 py-4 border-b border-champagne">
                        <h3 class="text-sm font-bold text-brown"><i class="fas fa-calculator mr-2 text-taupe"></i>Order Summary</h3>
                    </div>
                    <div x-show="calculating" class="px-5 py-6 text-center text-taupe">
                        <i class="fas fa-spinner fa-spin text-blue-500 mr-2"></i>Calculating…
                    </div>
                    <div x-show="!calculating" class="p-5">
                        <div x-show="!preview && items.length === 0" class="text-center text-taupe py-4 text-sm">
                        ...
                        <template x-if="preview || items.length > 0">
                            <div class="space-y-2 text-sm">
                                ...
                                <div class="border-t border-champagne pt-2 flex justify-between font-bold text-brown text-base">
                ...
                <p class="text-xs text-center text-taupe">Order will be created with status: <strong>Pending</strong></p>
            </div>
        </div>
    </div>
</div>
