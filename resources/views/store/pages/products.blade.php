...
<style>
    ...
    #activeFilterTags { display: flex; flex-wrap: wrap; }
</style>
@endpush
...
<div class="max-w-8xl mx-auto px-4 py-6 md:py-10">
    ...
    @if ($searchQuery)
        ...
        <header class="mb-8">
            <p class="text-xs font-semibold uppercase tracking-widest mb-2" style="color: var(--color-primary);">Search Results</p>
            ...
            <form method="GET" action="{{ route('products.index') }}" class="flex gap-2 max-w-xl">
                ...
                <button type="submit" class="btn-primary px-5 py-3 text-sm font-semibold cursor-pointer">Search</button>
            </form>
        </header>
    @else
        <header class="mb-8">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside id="catalogFilterPanel" class="hidden lg:block w-64 shrink-0 space-y-5">
            ...
            <div class="bg-white border border-champagne rounded-2xl p-5 shadow-sm">
                ...
                <div class="flex justify-between items-center mb-4">
                    <span id="priceMinDisplay" class="text-sm font-bold text-brown font-bengali bg-cream px-2.5 py-1 rounded-lg">
                        ৳{{ number_format(request('price_min', $priceRange['min'])) }}
                    </span>
                    <span class="text-xs text-taupe font-medium">to</span>
                    <span id="priceMaxDisplay" class="text-sm font-bold text-brown font-bengali bg-cream px-2.5 py-1 rounded-lg">
        ...
        <main class="flex-1 min-w-0">
            ...
            {{-- Toolbar --}}
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 bg-white p-4 rounded-2xl border border-champagne shadow-sm">
                <p class="text-muted text-sm">
                    ...
                    @if ($products->total() !== $products->count())
                        <span class="text-taupe">(page {{ $products->currentPage() }} of {{ $products->lastPage() }})</span>
                    @endif
                </p>
    ...
    <aside class="-translate-x-full absolute left-0 top-0 bottom-0 w-80 max-w-[90vw] bg-white overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div class="flex-1 p-5 space-y-6 overflow-y-auto">
                ...
                <div class="space-y-2.5">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="radio"
                               ...
                               {{ ! request('category') ? 'checked' : '' }}>
                        <span class="text-sm text-brown group-hover:text-primary transition-colors font-medium">All Categories</span>
                    </label>
                    @foreach ($globalCategories as $cat)
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="radio"
                                   ...
                                   class="w-4 h-4 accent-primary"
                                   {{ request('category') === $cat->slug || $selectedCategory?->id === $cat->id ? 'checked' : '' }}>
                            <span class="text-sm text-muted group-hover:text-primary transition-colors">{{ $cat->name }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex justify-between items-center mb-4">
                    <span id="priceMinDisplayMobile" class="text-sm font-bold text-brown font-bengali bg-cream px-2.5 py-1 rounded-lg">
                        ৳{{ number_format(request('price_min', $priceRange['min'])) }}
                    </span>
                    <span class="text-xs text-taupe">to</span>
                    <span id="priceMaxDisplayMobile" class="text-sm font-bold text-brown font-bengali bg-cream px-2.5 py-1 rounded-lg">
                        ৳{{ number_format(request('price_max', $priceRange['max'])) }}
                    </span>
                </div>
                <div class="catalog-range-wrapper mb-5">
