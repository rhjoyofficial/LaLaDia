<section class="py-16 px-4 md:px-8 bg-white overflow-hidden" id="allProducts">
    <div class="max-w-8xl mx-auto text-center">
        ...
        <h2 id="" class="font-heading text-2xl md:text-4xl font-bold text-brand mb-8">
            Shop By Categories
        </h2>
        ...
        <div class="mb-8 md:hidden px-2">
            <select onchange="filterCategory(this.value, this)"
                class="w-full border border-champagne rounded-full px-5 py-3 text-sm font-medium text-primary bg-cream focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div class="flex flex-wrap items-center justify-center gap-3">
                ...
                @foreach ($categories as $cat)
                    <button onclick="filterCategory({{ $cat->id }}, this)" data-category="{{ $cat->id }}"
                        class="category-tab px-6 py-2 rounded-full text-sm font-medium transition-all cursor-pointer whitespace-nowrap bg-cream text-primary border border-transparent hover:bg-gray-100">
                        {{ $cat->name }}
                    </button>
                @endforeach
            </div>
        ...
        <div id="categoryProductGrid" class="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-6 mb-12 min-h-112.5">
            ...
            @empty
                <div class="col-span-full py-12 text-taupe">
                    No products found.
                </div>
            @endforelse
        </div>
