                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                    ...
                    <div class="flex justify-center">
                        @if ($landing->hero_image)
                            <img src="{{ asset('storage/' . $landing->hero_image) }}" alt="{{ $combo->name }}"
                    ...
                    <div>
                        ...
                        @if ($combo->description)
                            <p class="text-cream text-lg mb-6">{{ $combo->description }}</p>
                        @endif

                        {{-- What's Inside --}}
                        <div class="mb-6">
                            <h3 class="text-sm font-semibold text-champagne mb-3 uppercase tracking-wider">What's Inside
                            </h3>
                            <div class="space-y-2">
                                @foreach ($combo->items as $comboItem)
                                    <div class="flex items-center gap-3 bg-gold-antique/30 rounded-xl px-4 py-2.5">
                                        <div class="w-2 h-2 rounded-full bg-gold-warm shrink-0"></div>
                                        <span class="text-sm">
                                            {{ $comboItem->variant->product->name ?? 'Product' }}
                                            &mdash; {{ $comboItem->variant->name ?? $comboItem->variant->sku }}
                                            <span class="text-gold-warm">x{{ $comboItem->quantity }}</span>
                                        </span>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="flex items-baseline gap-3">
                            ...
                            @if ($combo->compare_price && $combo->compare_price > $combo->price)
                                <span class="text-gold-warm line-through text-lg font-bengali">
