...
@section('content')
    <section class="min-h-screen bg-cream font-bengali">
        ...
        {{-- Body --}}
        <div class="max-w-6xl mx-auto px-4 py-12" data-lp-checkout data-lp-slug="{{ $landing->slug }}">
                    ...
                    <div class="space-y-3">
                        @foreach ($salesItems as $item)
                            ...
                            <div data-lp-item-card data-item-key="{{ $itemKey }}"
                                ...
                                data-combo-id="{{ $item->combo_id ?? '' }}" data-price="{{ $price }}"
                                data-tier-prices="{{ json_encode($tierData) }}" data-preselected="{{ $preselected }}"
                                ...
                                <div class="flex items-center gap-4 p-4">
                                    ...
                                    {{-- Checkbox indicator --}}
                                    <div data-lp-item-check data-active-class="bg-red-600 border-red-600"
                                        ...
                                        @if ($tierPrices->isNotEmpty())
                                            <div class="flex flex-wrap gap-1 mt-1">
                                                @foreach ($tierPrices->sortBy('min_quantity') as $tier)
                                                    ...
                                                    @endphp
                                                    <span
                                                        class="text-[10px] bg-red-50 text-red-600 border border-red-200 rounded-full px-2 py-0.5 font-semibold">
                ...
                <div class="w-full">
                    <div class="bg-white border border-red-100 rounded-3xl p-6 md:p-8 shadow-xl shadow-red-50/50">
                        ...
                        <div class="space-y-4">
                            ...
                            {{-- Bulk CTA --}}
                            <div
                                class="p-3.5 bg-amber-50 border border-dashed border-amber-300 rounded-xl flex items-center justify-between gap-3">
                                ...
                                <a href="tel:01334943783"
                                    class="text-xs font-black text-red-600 hover:underline shrink-0">কল করুন</a>
                            </div>
                            ...
                            <button data-lp-submit type="button"
                                ...
                                <svg data-lp-submit-spinner style="display:none" class="animate-spin h-5 w-5 text-white"
